...
class MaskedMultiHeadedSelfAttention(torch.nn.Module):
    def __init__(self, dim_model, n_heads, device='cpu'):
        """
        Initialize the Masked Multi-Headed Self-Attention module.
        ...
        """
        super(MaskedMultiHeadedSelfAttention, self).__init__()
        ...
        # Initialize the mask if provided
    def forward(self, x):
        """
        Forward pass for the Masked Multi-Headed Self-Attention module.
        ...
        for i in range(self.n_heads):
            q = self.linear_q[i](x) # Shape: (10, 50, 32)
            k = self.linear_k[i](x)
            v = self.linear_v[i](x)
    ...
    def scaled_dot_product_attention(self, q, k, v):
        ...
        scores = scores.masked_fill(mask, float('-inf')) # Apply the mask to the scores
